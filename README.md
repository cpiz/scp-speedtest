...
The script passes `LogLevel=ERROR` by default, and also passes `WarnWeakCrypto=no` when the local OpenSSH client supports it, to keep repeated ssh/scp warnings from cluttering speed test output. When a single-hop `ProxyJump` is detected, it is rewritten as a quiet `ProxyCommand` so jump-host warnings are suppressed too. Use `--show-ssh-warnings` to see those warnings.
