...
脚本默认会传入 `LogLevel=ERROR`，并在本地 OpenSSH 客户端支持时额外传入 `WarnWeakCrypto=no`，避免重复的 ssh/scp 安全提示刷屏。当检测到单跳 `ProxyJump` 时，脚本会把它改写成静默的 `ProxyCommand`，从而同时压制跳板机连接里的 warning。需要显示这些提示时，可以加 `--show-ssh-warnings`。
